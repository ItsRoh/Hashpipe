...
def evaluate_metrics(hw_top_k, ground_truth_file='ground_truth.json'):
    ...
    # 5. Print the Report
    print("\n" + "="*60)
    print(" 📊 HASHPIPE EVALUATION REPORT")
    print("="*60)
    ...
    for i in range(TOP_K):
        ...
        # Mark True Positives with a star for visual clarity
        marker = "⭐ " if i < len(hw_top_k) and hw_top_k[i][0] in true_positives else "   "
        print(f"{marker}#{i+1:<4} | {hw_str:<22} | {gt_str:<22}")

    print("\n[ Phase 1: Identification Metrics ]")
    print(f"✔️  True Positives  : {tp_count}")
